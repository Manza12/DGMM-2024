...
def plot_compare(spectrogram_1, spectrogram_2, name, title, images_folder,
    ...
    if paper is not None:
        ...
        fig.savefig(images_folder / (paper['name'] + '_0.pdf'), dpi=dpi, bbox_inches=bbox_0)
        fig.savefig(images_folder / (paper['name'] + '_1.pdf'), dpi=dpi, bbox_inches=bbox_1)
...
def plot_input_lines(lines, filtered_lines, spectrogram, images_folder, paper=None):
    ...
    if paper is not None:
        ...
        fig.savefig(images_folder / (paper['name'] + '.svg'), dpi=dpi)
...
def plot_input_paper(spectrograms, images_folder, settings):
    # Input spectrogram
    ...
    if settings.get('erosion', None) is not None:
        plot_compare(spectrograms['reconstruction_erosion'], spectrograms['erosion'],
                     ...
                     paper={'x_lim': (0.08, 0.16), 'y_lim': (500, 1000), 'name': 'erosion'})


def plot_noise_paper(spectrograms, images_folder, settings):
    # Opening spectrogram
...
def plot_sinusoids_paper(lines, spectrograms, images_folder, settings):
    # Vertical thinning spectrogram
...
def plot_transient_paper(lines, spectrograms, images_folder, settings):
    # Horizontal thinning spectrogram
...
def plot_output_paper(spectrograms, images_folder, settings):
    # Input - Output spectrogram
...
def plot_paper(lines, spectrograms, components, paths, settings):
    if components['input']:
        plot_input_paper(spectrograms, paths['images_folder'], settings)
    if components['noise']:
        plot_noise_paper(spectrograms, paths['images_folder'], settings)
    if components['sinusoids']:
        plot_sinusoids_paper(lines, spectrograms, paths['images_folder'], settings)
    if components['transient']:
        plot_transient_paper(lines, spectrograms, paths['images_folder'], settings)
    if components['output']:
        plot_output_paper(spectrograms, paths['images_folder'], settings)
