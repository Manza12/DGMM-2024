...
def create_stft_layer(verbose=False, normalize=False):
    if verbose:
    ...
    stft_layer.fs = FS
    if normalize:
        stft_layer.kernel_cos_inv = stft_layer.kernel_cos_inv / torch.sum(stft_layer.window_mask)
        stft_layer.kernel_sin_inv = stft_layer.kernel_sin_inv / torch.sum(stft_layer.window_mask)
        stft_layer.wcos = stft_layer.wcos / torch.sum(stft_layer.window_mask)
        stft_layer.wsin = stft_layer.wsin / torch.sum(stft_layer.window_mask)
...
def apply_stft_layer(signal, stft_layer, verbose=True, output_format='Magnitude', input_name='signal'):
    ...
    if output_format == 'Magnitude':
        ...
        spectrogram /= torch.sum(stft_layer.window_mask)
